  ...
  fetch('madrobby/microjs.com', 'master', 'data.js', force, function(e) {
    ...
    fs.stat(datajson, function(e) {
      ...
      fs.readFile(path.join(prefix, 'data.js'), 'utf8', function(e, body) {
        ...
        var result = vm.runInNewContext(body = body + '\n' + 'MicroJS');
        fs.writeFile(datajson, JSON.stringify(result, null, 2), function(e) {
          if(e) return cb(e);
          next('microjs');
        });
      });
...
guess.source = function source(from) {
  ...
  var source = Array.isArray(from.source) ? from.source : [from.source];
