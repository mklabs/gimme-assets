...
plugin.init = function(cb) {
  ...
  app.utils.mkdirp(app.get('prefix'), function(e) {
    if(e) return cb(e);
...
function load(force, cb) {
  ...
  // up to 20s, ok. If it exceeds assume a proxy environement with
  // no $http_proxy setup.
  var to = setTimeout(cb.bind({}, new Error(proxyErr)), 20000);
