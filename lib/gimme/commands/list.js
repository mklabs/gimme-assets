...
list.complete = function(o, cb) {
  var libs = packages('name', this.get('prefix'));

  if(/^--?\w?/.test(o.last)) return cb(null, ['--props', '--limit', '--bundle']);
  return cb(null, libs);
};

function list(opts, cb) {
  var terms = opts.args,
    cmd = opts.argv.remain[0],
    app = this,
    ln = terms.length;
  ...
  // ll -> long format, name, description, version homepage
  // la -> all format, all package props
  if(cmd === 'la') opts.props = ['true'];
  if(cmd === 'll') opts.props = ['name', 'description', 'version', 'homepage'];

  if(opts.bundle) libs = libs.filter(function(pkg) {
    return pkg.custom;
  });
...
function package(lib, prefix) {
  return packages(false, prefix).filter(function(p) { return p.name === lib; })[0];
}

function packages(key, prefix, dictionary) {
  dictionary = dictionary || 'gimme.json';
  var libs = JSON.parse(fs.readFileSync(path.join(prefix, dictionary), 'utf8')),
    bundles = dictionary !== 'bundle.json' ? JSON.parse(fs.readFileSync(path.join(prefix, 'bundle.json'), 'utf8')) : [];

  return libs.concat(bundles).map(function(pkg) {
