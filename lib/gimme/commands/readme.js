...
readme.usage = "gimme readme <name>";
readme.description = ['readme', "Show the appropriate documentation manpage generated from readme file"];
...
function readme (opts, cb) {
  ...
  app.info('Searching for', lib, '\'s readme');
...
function open(filepath, pkg, cb) {
  ...
  fs.readFile(path.join(__dirname, '../utils/templates/readme.html'), 'utf8', function(err, layout) {
    if(err) return cb(err);
