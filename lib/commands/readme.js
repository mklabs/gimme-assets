...
function readme (opts, cb) {
  ...
  //if(config.defaults.windows) return cb('Too bad.. manpages not supported on windows.');
  ...
  return gh.glob([userrepo, '*.md', '*.markdown', '*.mkd', 'README', 'readme', '*.txt'], {}, function(err, files) {
    ...
    var file = files.filter(function(f) { log.log(f); return /^readme/i.test(f); })[0];
    log.log('readme', file);
...
function man(filepath, pkg, cb) {
  if(config.defaults.windows) return open(filepath, cb);
