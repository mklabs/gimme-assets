  ...
  npm.commands.install(pkgs, function(err) {
    ...
    libs.forEach(function(lib) {
      var parts = lib.split('@'),
        name = parts[0],
        version = parts[1] || '*',
        pkg = path.join(find('node_modules', process.cwd(), cb), name),
...
function copy(from, to, cb) {
  exec('mkdir -p ' + to, function(err) {
    ...
    exec('cp -vr ' + from + ' ' + to, function(err, stdout) {
      if(err) return cb(err);
      console.debug(stdout.replace('\n', ''));
      cb();
    });
  });
}

function find(name, dir, cb) {
  ...
  if (dir === '/') {
    return cb(new Error('Could not find ' + name + ' up from: ' + dir));
  }

  return find(name, path.dirname(dir), cb);
}
